					...
					<tbody>
						...
						<c:forEach var="data" items="${LIST}">
							<tr>
								<td>${data.no}</td>
								<td><p class="blog_content ta__left">
									<a href="../Board/BoardView.do?nowPage=${PINFO.nowPage}&oriNo=${data.no}&kind=list">${data.title}</a> 
								</p></td>
								<td>${data.writer}</td>
								<td>${data.wday}</td>
								<td>${data.hit}</td>
						...
						<tr>
							<td align="center">
								...
								<c:if test="${PINFO.startPage ne 1}">
									<a href="../Board/BoardList.do?nowPage=${PINFO.startPage - 1}&kind=list">[이전]</a>
								</c:if>
								<%--	[1][2][3] --%>
								<c:forEach var="page" begin="${PINFO.startPage}" end="${PINFO.endPage}">
									<a href="../Board/BoardList.do?nowPage=${page}&kind=list">[${page}]</a>
								</c:forEach>
								<%--	[다음] --%>
								...
								<c:if test="${PINFO.endPage ne PINFO.totalPage}">
									<a href="../Board/BoardList.do?nowPage=${PINFO.endPage + 1}&kind=list">[다음]</a>
								</c:if>
							</td>
						</tr>
					</tbody>	
