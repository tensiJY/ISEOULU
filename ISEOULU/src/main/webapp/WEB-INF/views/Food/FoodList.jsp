...
ul li a:hover, ul li a:focus {  
	...
	<c:if test="${not empty FLIST}">
		...
		<c:forEach var="data" items="${FLIST}" varStatus="status">
			<c:if test="${data.mapx ne 9999}"> 
			markerPosition[${status.index}]  =  
				{
					content : '<div style="width:100px";><a href="../Food/View.do?nowPage=${nowPage}'+
					...
					<c:forEach var="data" items="${FLIST}" varStatus="status">
						...
						<div class="grid-3">
							<div class="gall_block">
								...
								<div class="gall_bot"><div class="text1"><a href="../Food/View.do?nowPage=${nowPage}&sgCode=${sgCode}&contentid=${data.contentid}&title=${data.title}&mapx=${data.mapx}&mapy=${data.mapy}">${data.title}</a></div>
									<br>
									주소 : ${data.addr1}<br>
...
<c:if test="${not empty FLIST}">				
	<ul>
		...
		<c:if test="${PINFO.startPage ne 1}">
			<li><a href='../Food/List.do?nowPage=${PINFO.startPage - 1}&sgCode=${sgCode}'>이전</a></li>
		</c:if> 
		<c:forEach var="page" begin="${PINFO.startPage}" end="${PINFO.endPage}">
			<li><a href="../Food/List.do?nowPage=${page}&sgCode=${sgCode}">${page}</a></li>
		</c:forEach>
		...
		<c:if test="${PINFO.endPage ne PINFO.totalPage}">
			<li><a href="../Food/List.do?nowPage=${PINFO.endPage + 1}&sgCode=${sgCode}">다음</a></li>
		</c:if>
	</ul>
</c:if>			
</section>
