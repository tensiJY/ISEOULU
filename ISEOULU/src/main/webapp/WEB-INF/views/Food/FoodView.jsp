	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(".btn_search").on('click', function(){
				...
				$(".btn_search").attr('href', '../Food/List.do?sgCode='+sgCode);
			});			
		...
		function imgOpen(a){
			window.open(a, "", "width=800 height=800");
		}
	</script>
...
<section class="content">
	<div class="container">
				<c:if test="${FVO.mapx ne 9999}"> 
				<input type="hidden" id="mapx" value="${FVO.mapx}">
				<input type="hidden" id="mapy" value="${FVO.mapy}">
				</c:if>
				<input type="hidden" id="sgCode" value="${FVO.sgCode}">
		<!--------------------- 상단 지역 검색 영역 --------------------->
		...
		<div class="row">
			<div class="grid_12">
				<h3>${FVO.title} (조회수 : ${FVO.readcount}) <a href="../Board/WriteForm.do?contentId=${FVO.contentid}"><img src="../resources/images/Sketch Book.png"></a></h3>

			</div>
		</div>
		<div class="row" >
			<div class="grid_55">
				<c:if test="${FVO.firstimage eq 'empty'}">
				<img src="" alt="" class="img_inner fleft inn__1">
				</c:if>
				<c:if test="${FVO.firstimage ne 'empty'}">
				<div id="imgDivSize"><img id="vimg" src="${FVO.firstimage}" alt="" onClick='imgOpen("${FVO.firstimage}")'></div>
				</c:if>
			</div>
			<!-- 사진 옆 오른쪽 스크롤 달아놨어요..!! -->
			<div class="grid_65" style="overflow:auto;  height:540px;">
				<div class="blog">
					...
					<tbody>
						<tr>
							<td>위치</td>
							<td><p class="blog_content">${FVO.addr1}</p></td>
						</tr>
						<tr>
							<td>문의/안내</td>
							<c:if test="${FVO.tel eq 'empty'}">
								<td>정보가 없습니다.</td>
							</c:if>
							<c:if test="${FVO.tel ne 'empty'}">
								<td>${FVO.tel}</td>
							</c:if>
						</tr>
				...
				<div class="blog">
					...
					<tbody>
						<tr>
							<td>개요</td>
							<td><p class="blog_content">${FVO.overview}</p></td>
						</tr>
						<tr>
							<td>정보제공자</td>
							<td>한국관광공사</td>
				...
				<div class="blog">
					...
					<tbody>
						...
						<c:if test="${FVO.treatmenu ne 'empty' }">
						<tr>
							<td>메뉴</td>
							<td>${FVO.treatmenu}</td>
						</tr>						
						...
						<tr>
							<td>영업시간</td>
							<c:if test="${FVO.opentimefood eq 'empty'}">
								<td>정보가 없습니다.</td>
							</c:if>
							<c:if test="${FVO.opentimefood ne 'empty'}">
								<td>${FVO.opentimefood}</td>
							</c:if>
						</tr>
					
						<c:if test="${FVO.parkingfood ne 'empty' }">
						<tr>
							<td>주차시설</td>
							<td>${FVO.parkingfood}</td>
						</tr>	
						</c:if>
							
						
						<c:if test="${FVO.smoking ne 'empty'}">
						<tr>
							<td>흡연석</td>
							<td>${FVO.smoking}</td>
						</tr>	
						...
						<c:if test="${FVO.reservationfood ne 'empty'}">
						<tr>
							<td>예약</td>
							<td>${FVO.reservationfood} <br></td>
						</tr>						
						</c:if>
					
						<c:if test="${FVO.seat ne 'empty' }">
						<tr>
							<td>좌석수</td>
							<td>${FVO.seat}</td>
						</tr>						
						...
						<c:if test="${FVO.kidsfacility ne 'empty' }">
						<tr>
							<td>어린이시설</td>
							<td>${FVO.kidsfacility}</td>
						</tr>						
						</c:if>																			
					</tbody>
					</table>
								...
								<div class="second">
									...
									<div class="margin-col">
										...
										<c:if test="${data.firstimage ne 'empty'}"><div class="imgDivSize2" ><img src="${data.firstimage}" alt="" class="img_inner noresize fleft"></div></c:if>		
											...
											<!-- 관광 -->
											<c:if test="${data.contenttypeid ne 39}"><c:if test="${data.contenttypeid ne 32}"><span class="txt_icon_mappoint">A</span>&nbsp;&nbsp;<span class="txt_icon_travel">여행지</span><div class="text10 color1"><a href="../Travel/View.do?sgCode=${data.siguncode}&contentid=${data.contentid}&title=${data.title}&mapx=${data.mapx}&mapy=${data.mapy}">${data.title}  [${data.round} m]</a></div></c:if></c:if>
											<!-- 음식-->
											<c:if test="${data.contenttypeid eq 39}"><span class="txt_icon_mappoint">B</span>&nbsp;&nbsp;<span class="txt_icon_restaurant">음식점</span><div class="text10 color1"><a href="../Food/View.do?sgCode=${data.siguncode}&contentid=${data.contentid}&title=${data.title}&mapx=${data.mapx}&mapy=${data.mapy}">${data.title} [${data.round} m]</a></div></c:if>
											<!-- 숙박 -->
											<c:if test="${data.contenttypeid eq 32}"><span class="txt_icon_mappoint">C</span>&nbsp;&nbsp;<span class="txt_icon_hotel">숙박</span><div class="text10 color1"><a href="../Hotel/View.do?sgCode=${data.siguncode}&contentid=${data.contentid}&title=${data.title}&mapx=${data.mapx}&mapy=${data.mapy}">${data.title} [${data.round} m]</a></div></c:if>
											...
											<c:if test="${data.round gt 700}">																	
												...
												onClick='searchWay(this,${FVO.mapx},${FVO.mapy},${data.mapx},${data.mapy}, "${FVO.title}", "${data.title}")'>여기까지 이동 정보&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;▼</span>
					...
					<div class="row">
						<div class="grid_115">
							<div class="content_bg_lightblue text4">
								주변 교통정보&nbsp;&nbsp;▶&nbsp;&nbsp;
								<span class="btn" onClick='pointSearch(${FVO.mapx},${FVO.mapy},2)'>지하철</span>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; 
								<span class="btn" onClick='pointSearch(${FVO.mapx},${FVO.mapy},1)'>버스</span>
							</div>
							<figure class=""><div id="map2" style="width:100%;height:540px;"></div></figure>
						</div>
						<div class="clear sep__2"></div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	mapOption = { 
	    center: new daum.maps.LatLng(${FVO.mapy}, ${FVO.mapx}),
	    level: 5// 지도의 확대 레벨
	};

	var map = new daum.maps.Map(mapContainer, mapOption);
	var markerPosition  = new daum.maps.LatLng(${FVO.mapy}, ${FVO.mapx});
...
var mkp  = new daum.maps.LatLng(${FVO.mapy}, ${FVO.mapx});
