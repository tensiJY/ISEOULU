	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(".btn_search").on('click', function(){
				...
				//nowPage
				$(".btn_search").attr('href', '../Travel/List.do?sgCode='+sgCode);
			});			
...
<section class="content">
	<div class="container">
				<c:if test="${TVO.mapx ne 9999}"><input type="hidden" id="mapx" value="${TVO.mapx}"><input type="hidden" id="mapy" value="${TVO.mapy}"></c:if>
				<input type="hidden" id="sgCode" value="${TVO.sgCode}">
		<!--------------------- 상단 지역 검색 영역 --------------------->
		<jsp:include page="../Common/searcharea.jsp"></jsp:include>
		
		<!-------------------- top. 상세정보 view 영역 start -------------------->
		<div class="row"><div class="grid_12"><h3>${TVO.title} (조회수 : ${TVO.readcount}) <a href="../Board/WriteForm.do?contentId=${TVO.contentid}"><img src="../resources/images/Sketch Book.png"></a></h3></div></div>
		<div class="row" >
			<div class="grid_55">
				<c:if test="${TVO.firstimage eq 'empty'}"><img src="" alt="" class="img_inner fleft inn__1"></c:if>
				<c:if test="${TVO.firstimage ne 'empty'}"><div id="imgDivSize"><img src="${TVO.firstimage}" alt="" onClick='imgOpen("${TVO.firstimage}")'></div></c:if>
			</div>
			...
			<div class="grid_65" style="overflow:auto;  height:540px;">
				<div class="blog">
					...
					<tbody>
						<tr>
							<td>위치</td>
							<td><p class="blog_content">${TVO.addr1}</p></td>
						</tr>
						<tr>
							<td>문의/안내</td>
							<c:if test="${TVO.tel eq 'empty'}">
								<td>정보가 없습니다.</td>
							</c:if>
							<c:if test="${TVO.tel ne 'empty'}">
								<td>${TVO.tel}</td>
							</c:if>
				...
				<div class="blog">
					...
					<tbody>
						<tr>
							<td>개요</td>
							<td><p class="blog_content">${TVO.overview}</p></td>
						</tr>
						<tr>
							<td>정보제공자</td>
							<td>한국관광공사</td>
				...
				<div class="blog">
					<table>
					<tbody>
						<c:if test="${TVO.chkbabycarriage ne 'empty' }"><tr><td>유모차 <br>대여 여부</td><td>${TVO.chkbabycarriage}</td></tr></c:if>
						<c:if test="${TVO.accomcount ne 'empty' }"><tr><td>수용인원</td><td>${TVO.accomcount}</td></tr></c:if>
						<c:if test="${TVO.chkcreditcard ne 'empty' }"><tr><td>신용카드가능여부</td><td>${TVO.chkcreditcard}</td></tr></c:if>
						<c:if test="${TVO.chkpet ne 'empty' }"><tr><td>애완동물가능여부</td><td>${TVO.chkpet}</td></tr></c:if>
						<c:if test="${TVO.expguide ne 'empty' }"><tr><td>체험안내</td><td>${TVO.expguide}</td></tr></c:if>
						<c:if test="${TVO.heritage1 ne 'empty' }"><tr><td>세계문화유산유무</td><td>${TVO.heritage1}</td></tr></c:if>
						<c:if test="${TVO.heritage2 ne 'empty' }"><tr><td>세계자연유산유무</td><td>${TVO.heritage2}</td></tr></c:if>
						<c:if test="${TVO.heritage3 ne 'empty' }"><tr><td>세계기록유산 유무</td><td>${TVO.heritage3}</td></tr></c:if>
						<c:if test="${TVO.infocenter ne 'empty' }"><tr><td>문의 및 안내</td><td>${TVO.infocenter}</td></tr></c:if>
						<c:if test="${TVO.opendate ne 'empty' }"><tr><td>개장일</td><td>${TVO.opendate}</td></tr></c:if>
						<c:if test="${TVO.parking ne 'empty' }"><tr><td>주차시설</td><td>${TVO.parking}</td></tr></c:if>
						<c:if test="${TVO.restdate ne 'empty' }"><tr><td>쉬는날</td><td>${TVO.restdate}</td></tr></c:if>
						<c:if test="${TVO.expagerange ne 'empty' }"><tr><td>체험가능 연령</td><td>${TVO.expagerange}</td></tr></c:if>
						<c:if test="${TVO.useseason ne 'empty' }"><tr><td>이용시기</td><td>${TVO.useseason}</td></tr></c:if>
						<c:if test="${TVO.usetime ne 'empty' }">
						<tr>
							<td>이용시간</td>
							<td>${TVO.usetime}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.agelimit ne 'empty' }">
						<tr>
							<td>관람 가능 연령</td>
							<td>${TVO.agelimit}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.bookingplace ne 'empty' }">
						<tr>
							<td>예매처</td>
							<td>${TVO.bookingplace}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.discountinfofestival ne 'empty' }">
						<tr>
							<td>할인정보</td>
							<td>${TVO.discountinfofestival}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.eventenddate ne 'empty' }">
						<tr>
							<td>행사 종료일</td>
							<td>${TVO.eventenddate}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.eventhomepage ne 'empty' }">
						<tr>
							<td>행사 홈페이지</td>
							<td>${TVO.eventhomepage}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.eventplace ne 'empty' }">
						<tr>
							<td>행사장소</td>
							<td>${TVO.eventplace}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.eventstartdate ne 'empty' }">
						<tr>
							<td>행사 시작일</td>
							<td>${TVO.eventstartdate}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.placeinfo ne 'empty' }">
						<tr>
							<td>행사장 위치안내</td>
							<td>${TVO.placeinfo}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.playtime ne 'empty' }">
						<tr>
							<td>공연시간</td>
							<td>${TVO.playtime}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.program ne 'empty' }">
						<tr>
							<td>행사 프로그램</td>
							<td>${TVO.program}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.spendtimefestival ne 'empty' }">
						<tr>
							<td>관람 소요시간</td>
							<td>${TVO.spendtimefestival}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.sponsor1 ne 'empty' }">
						<tr>
							<td>주최자 정보</td>
							<td>${TVO.sponsor1}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.sponsor1tel ne 'empty' }">
						<tr>
							<td>주최자 연락처</td>
							<td>${TVO.sponsor1tel}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.sponsor2 ne 'empty' }">
						<tr>
							<td>주관사 정보</td>
							<td>${TVO.sponsor2}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.sponsor2tel ne 'empty' }">
						<tr>
							<td>주관사 연락처</td>
							<td>${TVO.sponsor2tel}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.subevent ne 'empty' }">
						<tr>
							<td>부대행사</td>
							<td>${TVO.subevent}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.usetimefestival ne 'empty' }">
						<tr>
							<td>이용요금</td>
							<td>${TVO.usetimefestival}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.chkbabycarriageculture ne 'empty' }">
						<tr>
							<td>유모차대여여부</td>
							<td>${TVO.chkbabycarriageculture}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.accomcountculture ne 'empty' }">
						<tr>
							<td>수용인원</td>
							<td>${TVO.accomcountculture}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.chkcreditcardculture ne 'empty' }">
						<tr>
							<td>신용카드 <br>
							가능 여부</td>
							<td>${TVO.chkcreditcardculture}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.chkpetculture ne 'empty' }">
						<tr>
							<td>애완동물가능여부</td>
							<td>${TVO.chkpetculture}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.infocenterculture ne 'empty' }">
						<tr>
							<td>문의 및 안내</td>
							<td>${TVO.infocenterculture}</td>
						</tr>
						</c:if><c:if test="${TVO.parkingculture ne 'empty' }">
						<tr>
							<td>주차시설</td>
							<td>${TVO.parkingculture}</td>
						</tr>
						</c:if>
						
						<c:if test="${TVO.parkingfee ne 'empty' }">
						<tr>
							<td>주차요금</td>
							<td>${TVO.parkingfee}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.usefee ne 'empty' }">
						<tr>
							<td>이용요금</td>
							<td>${TVO.usefee}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.restdateculture ne 'empty' }">
						<tr>
							<td>쉬는날</td>
							<td>${TVO.restdateculture}</td>
						</tr>
						</c:if>
						<c:if test="${TVO.spendtime ne 'empty' }"><tr><td>관람 소요시간</td><td>${TVO.spendtime}</td></tr></c:if>
						<c:if test="${TVO.scale ne 'empty' }"><tr><td>규모</td><td>${TVO.scale}</td></tr></c:if>
						<c:if test="${TVO.usetimeculture ne 'empty' }"><tr><td>이용시간</td><td>${TVO.usetimeculture}</td></tr></c:if>
						<!-- 여행 코스는 테이블만 있고 데이터가 없지만 일단 넣었습니다.-->
						<c:if test="${TVO.distance ne 'empty' }"><tr><td>코스총거리</td><td>${TVO.distance}</td></tr></c:if>
						<c:if test="${TVO.infocentertourcourse ne 'empty' }"><tr><td>문의 및 안내</td><td>${TVO.infocentertourcourse}</td></tr></c:if>
						<c:if test="${TVO.schedule ne 'empty' }"><tr><td>코스일정</td><td>${TVO.schedule}</td></tr></c:if>
						<c:if test="${TVO.theme ne 'empty' }"><tr><td>코스테마</td><td>${TVO.theme}</td></tr></c:if>
						<c:if test="${TVO.taketime ne 'empty' }"><tr><td>코스 <br>총 소요시간</td><td>${TVO.taketime}</td></tr></c:if>
						<c:if test="${TVO.accomcountleports ne 'empty' }"><tr><td>수용인원</td><td>${TVO.accomcountleports}</td></tr></c:if>
						<c:if test="${TVO.chkbabycarriageleports ne 'empty' }"><tr><td>유모차대여여부</td><td>${TVO.chkbabycarriageleports}</td></tr></c:if>
						<c:if test="${TVO.chkcreditcardleports ne 'empty' }"><tr><td>신용카드가능여부</td><td>${TVO.chkcreditcardleports}</td></tr></c:if>
						<c:if test="${TVO.chkpetleports ne 'empty' }"><tr><td>애완동물가능여부</td><td>${TVO.chkpetleports}</td></tr></c:if>
						<c:if test="${TVO.expagerangeleports ne 'empty' }"><tr><td>체험 가능 연령</td><td>${TVO.expagerangeleports}</td></tr></c:if>
						<c:if test="${TVO.infocenterleports ne 'empty' }"><tr><td>문의 및 안내</td><td>${TVO.infocenterleports}</td></tr></c:if>
						<c:if test="${TVO.parkingfeeleports ne 'empty' }"><tr><td>주차요금</td><td>${TVO.parkingfeeleports}</td></tr></c:if>
						<c:if test="${TVO.parkingleports ne 'empty' }"><tr><td>주차시설</td><td>${TVO.parkingleports}</td></tr></c:if>
						<c:if test="${TVO.openperiod ne 'empty' }"><tr><td>개장시간</td><td>${TVO.openperiod}</td></tr></c:if>
						<c:if test="${TVO.reservation ne 'empty' }"><tr><td>예약안내</td><td>${TVO.reservation}</td></tr></c:if>
						<c:if test="${TVO.restdateleports ne 'empty' }"><tr><td>쉬는날</td><td>${TVO.restdateleports}</td></tr></c:if>
						<c:if test="${TVO.usefeeleports ne 'empty' }"><tr><td>입장료</td><td>${TVO.usefeeleports}</td></tr></c:if>
						<c:if test="${TVO.scaleleports ne 'empty' }"><tr><td>규모</td><td>${TVO.scaleleports}</td></tr></c:if>
						<c:if test="${TVO.usetimeleports ne 'empty' }"><tr><td>이용시간</td><td>${TVO.usetimeleports}</td>	</tr></c:if>
					</tbody>
					</table>
				</div>
			</div>
							...
							<c:if test="${data.contenttypeid eq 39}">
								<div class="second">
									...
									<div class="margin-col">
										...
										<c:if test="${data.firstimage ne 'empty'}"><div class="imgDivSize2" ><img src="${data.firstimage}" alt="" class="img_inner noresize fleft"></div></c:if>		
											<div class="extra_wrapper">
											<!-- 관광 -->
											<c:if test="${data.contenttypeid ne 39}">
												<c:if test="${data.contenttypeid ne 32}">
												<span class="txt_icon_mappoint">A</span>&nbsp;&nbsp;<span class="txt_icon_travel">여행지</span><br>											
												<div class="text10 color1"><a href="../Travel/View.do?sgCode=${data.siguncode}&contentid=${data.contentid}&title=${data.title}&mapx=${data.mapx}&mapy=${data.mapy}">${data.title} [${data.round} m]</a><br></div>
												</c:if>
											</c:if>
											<!-- 음식-->
											<c:if test="${data.contenttypeid eq 39}">
												<span class="txt_icon_mappoint">B</span>&nbsp;&nbsp;<span class="txt_icon_restaurant">음식점</span><br>
												<div class="text10 color1"><a href="../Food/View.do?sgCode=${data.siguncode}&contentid=${data.contentid}&title=${data.title}&mapx=${data.mapx}&mapy=${data.mapy}">${data.title} [${data.round} m]</a><br></div>				
											</c:if>
											<!-- 숙박 -->
											<c:if test="${data.contenttypeid eq 32}">
												<span class="txt_icon_mappoint">C</span>&nbsp;&nbsp;<span class="txt_icon_hotel">숙박</span><br>
												<div class="text10 color1"><a href="../Hotel/View.do?sgCode=${data.siguncode}&contentid=${data.contentid}&title=${data.title}&mapx=${data.mapx}&mapy=${data.mapy}">${data.title} [${data.round} m]</a><br></div>
											</c:if>
											...
											<%--700m 거리는 이동정보를 주지 않는다. --%>
											<c:if test="${data.round gt 700}">																	
												<span class="color1 ul" data-toggle="" data-target=""  
												onClick='searchWay(this,${TVO.mapx},${TVO.mapy},${data.mapx},${data.mapy}, "${TVO.title}", "${data.title}")'>여기까지 이동 정보&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;▼</span>
					...
					<div class="row">
						<div class="grid_115">
							<div class="content_bg_lightblue text4">
								주변 교통정보&nbsp;&nbsp;▶&nbsp;&nbsp;
								<!-- span 으로 수정함 -->
								<span class="btn" onClick='pointSearch(${TVO.mapx},${TVO.mapy},2)'>지하철</span>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; 
								<span class="btn" onClick='pointSearch(${TVO.mapx},${TVO.mapy},1)'>버스</span>
							</div>
							<figure class="">
...
<script type="text/javascript">
	...
	mapOption = { 
	    center: new daum.maps.LatLng(${TVO.mapy}, ${TVO.mapx}),
	    level: 5// 지도의 확대 레벨
	};

	var map = new daum.maps.Map(mapContainer, mapOption);
	var markerPosition  = new daum.maps.LatLng(${TVO.mapy}, ${TVO.mapx});
...
var mkp  = new daum.maps.LatLng(${TVO.mapy}, ${TVO.mapx});
