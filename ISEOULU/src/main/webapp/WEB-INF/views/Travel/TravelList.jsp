...
<head>
	...
	ul li a {display:-moz-inline-stack;  /*FF2*/display:inline-block;vertical-align:top;padding:4px;  
    /* margin-right:3px;   */
    width:40px !important;color:#000;font:bold 12px tahoma;border:3px solid #eee;text-align:center;text-decoration:none;width /**/:40px;    /*IE 5.5*/}  
					...
					<c:forEach var="data" items="${TLIST}" varStatus="status">
						...
						<div class="grid-3">
							<div class="gall_block">
								...
								<div class="gall_bot"><div class="text1"><a href="../Travel/View.do?nowPage=${nowPage}&sgCode=${sgCode}&contentid=${data.contentid}&title=${data.title}&mapx=${data.mapx}&mapy=${data.mapy}">${data.title}</a></div>
									<br>
									주소 : ${data.addr1}<br>
									조회수 : ${data.readcount} <br>
									<c:if test="${data.tel ne 'empty'}" > 전화번호 : ${data.tel} <br></c:if>
...
<c:if test="${not empty TLIST}">				
	<ul>
		...
		<c:if test="${PINFO.startPage ne 1}">
			<li><a href='../Travel/List.do?nowPage=${PINFO.startPage - 1}&sgCode=${sgCode}'>이전</a></li>
		</c:if> 
		<c:forEach var="page" begin="${PINFO.startPage}" end="${PINFO.endPage}">
			<li><a href="../Travel/List.do?nowPage=${page}&sgCode=${sgCode}">${page}</a></li>
		</c:forEach>
		...
		<c:if test="${PINFO.endPage ne PINFO.totalPage}">
			<li><a href="../Travel/List.do?nowPage=${PINFO.endPage + 1}&sgCode=${sgCode}">다음</a></li>
		</c:if>
	</ul>
</c:if>			
</section>
